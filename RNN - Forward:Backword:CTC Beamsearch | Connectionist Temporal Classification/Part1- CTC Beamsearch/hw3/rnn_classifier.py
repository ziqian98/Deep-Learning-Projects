    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            ...
            for l in range(1,self.num_layers):
                hidden[l] = self.rnn[l].forward(hidden[l-1],self.hiddens[i][l])
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in range(seq_len-1,-1,-1):
            for l in range(self.num_layers-1,-1,-1):
                ...
                if l!=0:
                    temp,dh[l] = self.rnn[l].backward(dh[l],self.hiddens[i+1][l],
                                                     self.hiddens[i+1][l-1],self.hiddens[i][l])
                    dh[l-1] = dh[l-1] + temp
                else:
                    _,dh[l]= self.rnn[l].backward(dh[l],self.hiddens[i+1][l],self.x[:,i],self.hiddens[i][l])
