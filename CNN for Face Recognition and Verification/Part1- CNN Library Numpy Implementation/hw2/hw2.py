...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            self.convolutional_layers.append(Conv1D(in_channel = num_input_channels,
            ...
            input_width = (input_width-kernel_sizes[i]+strides[i]) // strides[i]
            num_input_channels = num_channels[i]

        self.linear_layer = Linear(in_feature=num_input_channels*input_width,
    ...
    def forward(self, x):
        ...
        for i in range(self.nlayers):
            x = self.activations[i].forward(self.convolutional_layers[i].forward(x))
        x = self.flatten.forward(x)
        self.output = self.linear_layer.forward(x)
    ...
    def backward(self, labels):
        ...
        for i in range(self.nlayers-1, -1, -1):   # i from len-1 to 0 with step 1
            grad = self.convolutional_layers[i].backward(np.multiply(grad, self.activations[i].derivative()))
        return grad
