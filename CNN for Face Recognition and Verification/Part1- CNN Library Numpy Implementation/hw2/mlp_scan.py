...
class CNN_SimpleScanningMLP():
    ...
    def init_weights(self, weights):
        ...
        w1,w2,w3 = weights # w1 = 192 * 8, w2 = 8 * 16, w3 = 16 * 4
...
class CNN_DistributedScanningMLP():
    ...
    def init_weights(self, weights):
        ...
        w1, w2, w3 = weights
        self.conv1.W = np.transpose(np.reshape(np.transpose(w1[:,:2]),(2,8,24))[:,:2,:],(0,2,1)) #2*24*2
        self.conv2.W = np.transpose(np.reshape(np.transpose(w2[:,:8]),(8,4,2))[:,:2,:],(0,2,1)) #8*2*2
        self.conv3.W = np.transpose(np.reshape(np.transpose(w3),(4,2,8)),(0,2,1)) #4*8*2
